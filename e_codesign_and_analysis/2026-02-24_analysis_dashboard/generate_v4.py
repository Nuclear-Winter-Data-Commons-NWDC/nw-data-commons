#!/usr/bin/env python3
"""
Generate analysis_pivot_v4.html with improved chart gridlines and axis controls.

Changes from v3:
1. Higher contrast gridlines (lighter grey)
2. X-axis min/max controls added
3. Conditional greying: Y-axis controls greyed in horizontal bar mode, X-axis controls greyed in column/line mode
"""
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  input[type=color] { width: 60px; height: 34px; background: var(--bg3); border: 1px solid var(--border); border-radius: 6px; cursor: pointer; }
  ...
  .loading-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(13, 17, 23, 0.85); display: none; align-items: center; justify-content: center; z-index: 1000; flex-direction: column; gap: 16px; }
...
<div class="loading-overlay" id="loading-overlay">
  ...
  <div class="loading-text">Generating pivot table and chart...</div>
</div>

<div class="header">
  <h1>🔬 Nuclear Winter Data Commons — Analysis Dashboard</h1>
  <div class="subtitle">Interactive Pivot Table & Charts for Starvation Data</div>
</div>
...
<script>
let STARV_DATA=null,ALL_FIELDS=[],PIVOT_RESULTS=null,CHART_INSTANCE=null,updateTimer=null;
const pivotConfig={rows:[],columns:[],filters:{}};

fetch('starvation_data.json').then(r=>r.json()).then(data=>{
  STARV_DATA=data; ALL_FIELDS=Object.keys(data[0]||{}); initializeInterface();
}).catch(err=>{console.error(err);document.getElementById('result-info').textContent='Error loading data.';});
...
function initializeInterface(){
  ...
  ALL_FIELDS.forEach(f=>{
    ...
    if(f==='pct.population.starving.2010')o1.selected=true;
    if(f==='country.population.2010')o2.selected=true;
...
function updateAxisControlsState(){
  ...
  // In horizontal bar: X controls enabled (values), Y controls disabled (categories)
  // In column/line: Y controls enabled (values), X controls disabled (categories)
  document.getElementById('x-min-group').classList.toggle('disabled',!isHorizontalBar);
  document.getElementById('x-max-group').classList.toggle('disabled',!isHorizontalBar);
...
function addFieldToZone(f,z){
  if(z==='filters'){
    ...
    const uv=new Set();STARV_DATA.forEach(r=>uv.add(String(r[f]||'')));
...
function renderZone(z){
  ...
  if(z==='filters'){
    Object.keys(pivotConfig.filters).forEach(f=>{
      ...
      fc.appendChild(fh);
      const av=new Set();STARV_DATA.forEach(r=>av.add(String(r[f]||'')));
...
function selectAllFilterValues(f){const a=new Set();STARV_DATA.forEach(r=>a.add(String(r[f]||'')));pivotConfig.filters[f]=a;renderZone('filters');scheduleUpdate();}
...
function updateChart(){
  ...
  const {data,colKeys}=PIVOT_RESULTS,rk=Object.keys(data).sort();
  const ct=document.getElementById('chart-type').value;
  ...
  const xAxisConfig={
    ...
    min:isHorizontalBar&&xm?parseFloat(xm):undefined,
    max:isHorizontalBar&&xM?parseFloat(xM):undefined
  };
  const yAxisConfig={
    grid:{color:'#3a404a',borderColor:'#3a404a'},
    beginAtZero:false,
...
function resetChart(){
  ...
  document.getElementById('x-min').value='';document.getElementById('x-max').value='';
  document.getElementById('y-min').value='';document.getElementById('y-max').value='';
...
with open('analysis_pivot_v4.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print("✓ Generated analysis_pivot_v4.html")
print("Changes:")
print("  - Higher contrast gridlines (color: #3a404a vs bg: #0d1117)")
print("  - X-axis min/max controls added")
print("  - Conditional greying: Y-axis disabled in horizontal bar mode, X-axis disabled in column/line mode")
print("  - updateAxisControlsState() function manages control state based on chart type")
