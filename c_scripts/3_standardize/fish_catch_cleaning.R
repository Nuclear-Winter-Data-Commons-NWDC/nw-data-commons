...
# ---------------------------------------------------------------------------
# This section uses DetectAndImportData() to flexibly handle:
# - Multiple CSV files (one per scenario), OR
...
if (fish_catch_import$file_type == "none" || is.null(fish_catch_import$data)) {
  stop("No data files found in fish_catch directory: ", fish_catch_dir)
}
