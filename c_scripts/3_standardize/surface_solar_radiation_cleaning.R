...
if (ssr_import$file_type == "mixed") {
  stop("Mixed file types detected in surface_solar_radiation directory. ",
       "Please ensure directory contains ONLY CSVs OR ONLY Excel files.")
}

surface.solar.radiation.ls <- ssr_import$data
ssr.sheet.names <- names(surface.solar.radiation.ls)

# Function to clean and reshape a single surface solar radiation table
CleanReshape_SSR <- function(source_table, source_table_name) {
  ...
  indicator <- dplyr::case_when(
    grepl("mean_max", source_table_name) ~ "surface.solar.radiation.max",
    grepl("mean_min", source_table_name) ~ "surface.solar.radiation.min",
    grepl("mean_stdev", source_table_name) ~ "surface.solar.radiation.stdev",
    grepl("_mean$", source_table_name) ~ "surface.solar.radiation.mean",  # matches after version suffix stripped by ImportCSVsFromDirectory
    TRUE ~ NA_character_
  )
...
# Process all tables
surface.solar.radiation.clean.tb <-
  Map(
    CleanReshape_SSR,
    surface.solar.radiation.ls,
    ssr.sheet.names
  ) %>%
  do.call(rbind, .) %>%
  pivot_wider(
  ...
  FlagOutliers_IQR(source.table.list.name = surface.solar.radiation.ls) %>%
  dplyr::select( # select & order final variables
    any_of(c(
      ...
      "surface.solar.radiation.mean", "surface.solar.radiation.mean.outlier.flag",
      "surface.solar.radiation.min", "surface.solar.radiation.min.outlier.flag",
      "surface.solar.radiation.max", "surface.solar.radiation.max.outlier.flag",
      "surface.solar.radiation.stdev", "surface.solar.radiation.stdev.outlier.flag"
    ))
  ) %>%
  as_tibble()
...
if (interactive()) {
  surface.solar.radiation.clean.tb %>% as.data.frame() %>% .[sample(1:nrow(.), 10), ]
}
